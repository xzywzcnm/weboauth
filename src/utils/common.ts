 ...
 */
  ...
  getPower (keys:string|Array<string>|{[key:string]:string}):boolean | {[key:string]:boolean} {
    ...
    const isPass:boolean = !this.isEmpty(userInfo.loginName);
    ...
    if (this.isObject(keys)) {
      ...
      if (this.isJson(keys)) {
        ...
        for (let i = 0, len = objKeys.length; i < len; i++) {
          newVal[objKeys[i]] = isPass || permissionsIds.includes(keys[objKeys[i]]);
        }
      } else if (this.isArray(keys)) {
        for (let i = 0, len = keys.length; i < len; i++) {
          newVal[keys[i]] = isPass || permissionsIds.includes(keys[i]);
        }
      }
      return newVal;
    }

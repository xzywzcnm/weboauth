...
const getTargetContainer = (targetContainer:string | Element = '') => {
  ...
  if (typeof targetContainer === 'string') {
    const getDom = document.querySelectorAll(targetContainer);
    if (!global.$common.isEmpty(getDom)) {
      // 转成 Array 类型
      container = [].slice.call(getDom);
    }
  } else {
    container = targetContainer ? [targetContainer] : [];
  }
  container.forEach(demo => {
    imgDom = demo.querySelectorAll('img');
    if (!global.$common.isEmpty(imgDom)) {
      imgList = [...imgList, ...[].slice.call(imgDom)];
    }
  });
  return imgList;
};
